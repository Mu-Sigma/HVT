...
list.of.packages <- c("dplyr", "kableExtra", "geozoo", "plotly", "purrr", "HVT",  "data.table", "gridExtra","plyr")
new.packages <-
...
This is a custom-built function called `Table`, designed to neatly display all tabulated results throughout the vignette.

```{r}
scrollLimit <- function(noOfRows){
  if(noOfRows<10){
...
Table <- function(data,scroll = FALSE, limit = NULL){
  ...
  scroll <- scroll
  if(scroll == TRUE){
  kable_table <- kable_table %>% scroll_box(width = "100%", height = scrollLimit(nrow(data)))}
 return(kable_table)
}
...
```{r torus hvt first,warning=FALSE,fig.show='hold',results='hide',message=FALSE}
hvt.torus <- trainHVT(
  torus_train,
  n_cells = 100,
  depth = 1,
...
```{r torus hvt second,warning=FALSE,fig.show='hold',results='hide',message=FALSE}
hvt.torus2 <- trainHVT(
  torus_train,
  n_cells = 300,
  depth = 1,
...
```{r compression summary torus second,warning=FALSE}
...
```{r torus hvt third ,warning=FALSE,fig.show='hold',results='hide',message=FALSE}
set.seed(240)
hvt.torus3 <- trainHVT(
  torus_train,
  n_cells = 500,
...
```{r scoreHVT torus,warning=FALSE,message=FALSE}
set.seed(240)
...
```{r level one computers,warning=FALSE,message=FALSE,results='asis'}
...
```{r compression summary level one computers,warning=FALSE}
...
```{r scoreHVT hmap computers,warning=FALSE,message=FALSE}
set.seed(240)

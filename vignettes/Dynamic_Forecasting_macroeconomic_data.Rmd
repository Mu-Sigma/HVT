...
The latest version introduces an enhancement to dynamic forecasting with the release of **raw series ex-ante forecasting**. In this approach, a **simple 12-month direct lookback method** is used, where raw historical values combined with the forecasted **year-over-year (YoY)** changes are applied to generate raw-level forecasts for each feature over the ex-ante period.
...
The value `r round(hvt.results[[3]]$compression_summary$percentOfCellsBelowQuantizationErrorThreshold,2)`, indicates that **`r round((hvt.results[[3]]$compression_summary$percentOfCellsBelowQuantizationErrorThreshold),2) * 100`% compression** has been achieved, Typically, the number of cells is increased  atleast 80% compression is achieved. However, for this vignette, we proceed with the current level of compression, as this specific combination of cells and dataset intentionally induces problematic transition states, which are useful for demonstrating the proposed solution.
...
The centroid of a cell is the average value of all the records within that cell in the multidimensional space. Below is the table displaying the centroids of all 75 cells of the trained HVT Model. In this table, *n* represents the number of records contained within each cell.
...
- The x-axis represents the time period, and the y-axis represents the states (in states plot) and raw values in transformed units (means the values are log diffed and scaled in trainHVT) in feature plots. 
...
- Similar to the Ex-post plots, but without the actual data, showing only the predicted states. The x-axis represents the time period, and the y-axis represents the states (in states plot) and raw values (in feature plots). 
...
Let’s re-run the ex-post simulation with `handle_problematic_states = TRUE` to identify whether any states are problematic and understand the underlying cause with 6 clusters and 3 nearest neighbors.
...
Below is the model dendrogram, where the 75 cells are grouped into an optimal **k = 6 clusters**. For each problematic state, **3 nearest neighbors** are identified, providing potential proximate states based on Euclidean distance to transition to when the simulation becomes stuck in a flat trajectory.
...
With all problematic states now addressed, we can review the updated simulations, which no longer exhibit flatline behavior and instead show meaningful fluctuations and more informative forecast dynamics.
...
Below is the MAE (Mean Absolute Deviation) values for each feature and state. It is calculated as the absolute difference between the actual and predicted median, providing insights into the model's accuracy and performance. The Mean MAE is calculated only from the features not states.
...
```{r, fig.width=10, out.width='100%',fig.height = 6, fig.align='center'}
ex_ante_raw[["plots"]]$CPI_Food
```

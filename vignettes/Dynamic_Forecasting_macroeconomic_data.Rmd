...
This notebook introduces a new feature msm in the HVT package, which stands for Monte Carlo Simulations of Markov Chain — a feature designed for Dynamic Forecasting of time series data, along with Simulation Density Plot and Reconciliation Table for enhanced visualization and result validation.
...
Additionally, a Simulation Density Plot is introduced to visually represent how simulation paths overlap over time. The color-coded density levels help identify regions of high or low simulation concentration, offering deeper insight into model behavior and uncertainty.

A Reconciliation Table is also included to compare and align forecasted values with actual observations, helping to assess consistency and correctness across different forecast states.
...
3)Simulation Density Plot : A high-resolution visualization that maps the density of simulated trajectories using ggpointdensity. Each line represents one simulation path, while the color intensity (blue → yellow → red) indicates how frequently simulations pass through that region of the forecast space. This plot helps to visually identify regions of convergence or divergence among simulation paths and offers an intuitive understanding of model uncertainty across time.

4)Reconciliation Table : A tabular summary showing how many simulations occupy the same (time, value) position. This table helps quantify the clustering and convergence patterns observed in the density plot.
...
### Density Simulation Plot and Reconcilation Table {.tabset}

A visual and tabular summary of simulated trajectories showing the distribution of simulations over time (density plot) and the exact counts of simulation paths per state at each timestamp (reconciliation table).

##### density plot
```{r, fig.width=10.5,out.width='100%', fig.height=6, fig.align='center'}
ex_post[["density_plot"]]
```

##### reconcilation table

```{r, fig.width=8.5,out.width='100%', fig.height=4.5, fig.align='center'}
recon_table <- ex_post[["reconciliation_table"]]
recon_table$Timestamp <- as.character(recon_table$Timestamp)
dynamic_length_menu <- calculate_dynamic_length_menu(nrow(recon_table))
datatable(recon_table, options = list(pageLength = 10,scrollX = TRUE,lengthMenu = dynamic_length_menu,columnDefs = list(list(width = '150px', targets = 1))),class = "nowrap display",rownames = FALSE)
```
...
##### density plot
```{r, fig.width=9.5,out.width='100%', fig.height=6, fig.align='center'}
ex_ante[["density_plot"]]
```

##### reconcilation table

```{r, fig.width=8.5,out.width='100%', fig.height=4.5, fig.align='center'}
recon_table <- ex_ante[["reconciliation_table"]]
recon_table$Timestamp <- as.character(recon_table$Timestamp)
dynamic_length_menu <- calculate_dynamic_length_menu(nrow(recon_table))
datatable(recon_table, options = list(pageLength = 10,scrollX = TRUE,lengthMenu = dynamic_length_menu,columnDefs = list(list(width = '150px', targets = 1))),class = "nowrap display",rownames = FALSE)
```

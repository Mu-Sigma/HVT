...
```{r,warning=FALSE, message=FALSE, results='hide'}
max_cells <- (nrow(train_dataset)/2)
...
From the results, the Mean of Variables MAE is `r table_mae`, while the MAE for the same combination from HVTMSMoptimization is `r best_mae`, and the two values are quite close. Since the `msm` process involves random number generation for simulation to select the next state, an **error range of 0.02 to 0.1** occurs.

...
list.of.packages <- c("HVT", "dplyr","magrittr", "ggplot2", "kableExtra", "gridExtra","grid", "httr", "stats", 
...
```{r sammons, warning=FALSE, message=FALSE}
...
```{r}
displayTable(data= hvt.results$model_info$distance_measures)
```
...
```{r}
displayTable(scoring[["scoredPredictedData"]], value = 0.7) 
```
...
hclust_plot_1 <- clustHVT(dataset = centroid_cluster_data, hvt.results = hvt.results, domains.column = "cluster_name", type = "plot")
```
...
```{r,message=FALSE, warning=FALSE, fig.width=8.8, fig.height = 5,fig.align = "center"}
centroid_cluster_data <- cluster_data %>% dplyr::select('Cell.ID', 'cluster_name','names.column')
manual_plot_1 <-clustHVT(dataset = centroid_cluster_data, hvt.results = hvt.results, domains.column = "cluster_name", type = "plot")
```
...
centroid_cluster_data <- cluster_data %>% dplyr::select('Cell.ID', 'cluster_name','names.column')
hclust_plot_2 <- clustHVT(dataset = centroid_cluster_data, hvt.results = hvt.results, domains.column = "cluster_name", type = "plot")
...
```{r,message=FALSE, warning=FALSE, fig.width=8.8, fig.height = 5,fig.align = "center"}
centroid_cluster_data <- cluster_data %>% dplyr::select('Cell.ID', 'cluster_name','names.column')
manual_plot_2 <- clustHVT(dataset = centroid_cluster_data, hvt.results = hvt.results, domains.column = "cluster_name", type = "plot")
```

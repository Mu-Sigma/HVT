...
get_cluster_domains <- function(k, iterations = 10) {
 ...
 filepath <- file.path("./twitter_data/", filename)
  ...
  formatted_table <- displayTable(results, scroll = TRUE)
...
displayTable(data[1:10,], scroll = T)
```
...
```{r}
displayTable(dataset_raw[1:10,], scroll = T)
```
...
```{r}
displayTable(dataset[1:10,],scroll = T)
```
...
Act_pred_Table[1:10,]  %>%as.data.frame() %>%displayTable(scroll = TRUE) 
```
...
```{r}
displayTable(hclust_data_1, scroll = T)
```
...
```{r}
displayTable(hclust_data_2[1:10,], scroll = T)
```
